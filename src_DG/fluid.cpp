...
struct GaussQuadrature {
    // 存储 2阶 到 5阶 的高斯-勒让德节点 (Gauss-Legendre Points)
    // 范围: [-1, 1]
    static constexpr double POINTS[6][5] = {
        {}, // 0阶 (不使用)
        {0.0, 0, 0, 0, 0}, // NQ=1 (如果需要 $P^0$ 时使用)
        {-0.5773502691896257, 0.5773502691896257, 0, 0, 0}, // NQ=2
        {-0.7745966692414834, 0.0, 0.7745966692414834, 0, 0}, // NQ=3
        {-0.8611363115940526, -0.3399810435848563, 0.3399810435848563, 0.8611363115940526, 0}, // NQ=4
        {-0.9061798459386640, -0.5384693101056831, 0.0, 0.5384693101056831, 0.9061798459386640} // NQ=5
    };

    // 存储 2阶 到 5阶 的权重 (Weights)
    static constexpr double WEIGHTS[6][5] = {
        {}, 
        {2.0, 0, 0, 0, 0}, // NQ=1
        {1.0, 1.0, 0, 0, 0}, // NQ=2
        {0.5555555555555556, 0.8888888888888888, 0.5555555555555556, 0, 0}, // NQ=3
        {0.3478548451374538, 0.6521451548625461, 0.6521451548625461, 0.3478548451374538, 0}, // NQ=4
        {0.2369268850561891, 0.4786286704993665, 0.5688888888888889, 0.4786286704993665, 0.2369268850561891} // NQ=5
    };
};
// 使用示例：
...
void applyLimiter(Mesh& mesh) 
{
    ...
    double s0;
    if (DG_P >= 3) s0 = -3.0* log10((double)DG_P); 
